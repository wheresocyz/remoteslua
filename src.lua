...
-- Demo data for testing if nothing exists
local function addDemo()
	if #game:GetDescendants() == 0 then
		...
		v.Name = "Health"
		v.Value = 100
		v.Parent = player.Character or player
	end
end
addDemo()
...
for i,v in ipairs(tabs) do
	...
	b.MouseLeave:Connect(function()
		if currentTab~=v then
			TweenService:Create(b,TweenInfo.new(0.2),{BackgroundTransparency=0.4}):Play()
		end
	end)
...
local function parse(text)
	...
	for p in string.gmatch(text,"[^,]+") do
		...
		elseif p=="true" then table.insert(args,true)
		elseif p=="false" then table.insert(args,false)
		else table.insert(args,p)
		end
	end
	return args
end

local function logAction(t)
	table.insert(logs,1,t)
...
function scan()
	...
	for _,v in ipairs(game:GetDescendants()) do
		if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") or v:IsA("ValueBase") then table.insert(items,v) end
	end
	buildList()
end
...
runBtn.MouseButton1Click:Connect(function()
	...
	if selected:IsA("RemoteEvent") then
		selected:FireServer(unpack(parse(input.Text)))
		logAction("Fired "..selected.Name.." | "..input.Text)
	elseif selected:IsA("RemoteFunction") then
		local r=selected:InvokeServer(unpack(parse(input.Text)))
